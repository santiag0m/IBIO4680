...
% PHOW_IMAGENET Image classification in the ImageNet dataset
%   This program demonstrates how to use VLFeat to construct an image
%   classifier on the ImageNet data. The classifier uses PHOW
...
%
%   Call PHOW_IMAGENET to train and test a classifier on a small
%   subset of the ImageNet data. Note that the program
%   automatically downloads a copy of the ImageNet data from the
%   Internet if it cannot find a local copy.
%
%   Edit the PHOW_IMAGENET file to change the program configuration.
%
%   To run on the entire dataset change CONF.TINYPROBLEM to FALSE.
%
%   The ImageNet data is saved into CONF.CALDIR, which defaults to
%   'data/imageNet200'. Change this path to the desired location, for
%   instance to point to an existing copy of the ImageNet data.
%
...
%
%
% Modified for Universidad de los Andes IBIO4680 course (Added
% compatibility for ImageNet200)
...
if ~exist(conf.calDir, 'dir') || ...
   (~exist(fullfile(conf.calDir, 'acorn'),'dir') && ...
    ~exist(fullfile(conf.calDir, 'train', 'acorn')))
...
else % Add new images to the current dataset
...
if ~exist(conf.histPath) || conf.clobber
  ...
  if ~testFolder
      ...
      parfor ii = 1:lastTrain
      % for ii = 1:length(images)
        fprintf('Processing %s (%.2f %%)\n', images{ii}, 100 * ii / length(images)) ;
        im = imread(fullfile(conf.calDir, images{ii})) ;
        hists{ii} = getImageDescriptor(model, im);
      end
      parfor ii = lastTrain+1:length(images)
      % for ii = 1:length(images)
        fprintf('Processing %s (%.2f %%)\n', images{ii}, 100 * ii / length(images)) ;
        im = imread(fullfile(conf.calTestDir, images{ii})) ;
        hists{ii} = getImageDescriptor(model, im);
      end
...
disp('Model Accuracy:')
disp(100 * mean(diag(confus)/conf.numTest))
